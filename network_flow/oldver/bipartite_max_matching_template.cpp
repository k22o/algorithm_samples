...
int main(){
    ll X,Y,E,u,v;
    cin >> X >> Y >> E;
    capacity.assign((X+Y+2),vector<ll>((X+Y+2),0));
    flow.assign((X+Y+2),vector<ll>((X+Y+2),0));

    //startノードは[X+Y],targetノードは[X+Y+1]
    rep(i,0,E){
        cin >> u >> v ;
        capacity[u][X+v] = 1;
        capacity[X+Y][u] = 1;
        capacity[X+v][X+Y+1] = 1;        
    }
    cout << edmonds_karp(X+Y,X+Y+1) << endl;
}
