...
class LazySegmentTree{
    private:
        ...
        vector<bool> lazyBool;
        ll last_node;//最終層のノード数

    public:
        LazySegmentTree(ll n, ll init_tree= INF, ll init_lazy=INF){
            this-> last_node = 1;
            ...
            this->lazyBool.assign((this->last_node*2-1),false);
        };

    // ノードkに対し，遅延配列から値を伝搬する
    void eval(ll k, ll l, ll r){
        if(this->lazyBool[k]==true){
            this->tree[k] = this->lazy[k];
            if(r-l>1){
                this->lazy[2*k+1] = this->lazy[k];
                this->lazy[2*k+2] = this->lazy[k];
                this->lazyBool[2*k+1] = true;
                this->lazyBool[2*k+2] = true;
            }
            this->lazyBool[k]=false;
        }
    };
    
    // aからbまでのノードをwに書き換える
    void lazyUpdate(ll a, ll b, ll w, ll k=0,ll l=0, ll r=-1){
        ...
        if(a<=l && r<=b){
            this->lazy[k] = w;
            this->lazyBool[k] = true;
            eval(k,l,r);
        }
        //そうでない
        else{
            this->lazyUpdate(a,b,w,2*k+1,l,(l+r)/2);
            this->lazyUpdate(a,b,w,2*k+2,(l+r)/2,r);
    ...
    ll getMin(ll a, ll b, ll k=0, ll l=0, ll r=-1){
        ...
        if(a<=l && r<=b) return this->tree[k];
            ...
            ll tmp1 = this->getMin(a,b,2*k+1,l,(l+r)/2);
            ll tmp2 = this->getMin(a,b,2*k+2,(l+r)/2,r);
...
int main(){
    ...
    rep(i,0,q){
        ...
        if(query==0){
            cin >> s >> t >> x;
            tree.lazyUpdate(s,t+1,x);
        }else{
            cin >> s >> t;
            ans.push_back(tree.getMin(s,t+1));
        }
    }
