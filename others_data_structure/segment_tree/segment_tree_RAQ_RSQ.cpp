...
class SegmentTree{
    ...
    public:
        SegmentTree(ll n, ll init_num= 0){
            this-> N = n;
            this-> last_node = 1;
    ...
    //もとの配列のidx番目に要素にvalueを足し，seg木を更新する
    ...
    //[start,fin)の区間について総和を求める
    ll segFind(ll start, ll fin, ll k=0, ll l=0, ll r=-1){
        if(r<=start||fin<=l) return 0;
        else if (start<=l && fin>=r) return this->tree[k];
        else{
            ll c1 = this->segFind(start,fin,2*k+1,l,(l+r)/2);
            ll c2 = this->segFind(start,fin,2*k+2,(l+r)/2,r);
            return c1+c2;
        }
    };
...
int main(){
    ...
    rep(i,0,q){
        if(com[i]==0) segTree.segUpdate(x[i]-1,y[i]);
        else if (com[i]==1){
            ll ans = segTree.segFind(x[i]-1,y[i],0,0,segTree.ret_last_node());
            cout << ans << endl;
        }
    }
    /*   
